...
| `weflow.reconnect.intervalSec` | 自动重连间隔(秒，固定不退避) | integer | `1` | 是 | | FR-CONN-04 |
| `weflow.reconnect.logIntervalSec` | 重连日志汇总周期(秒) | integer | `30` | 是 | | FR-CONN-04 |

> 重连策略已由「指数退避」改为「固定间隔循环」：每 `intervalSec` 重跑一次三级连接判定（health → SSE → 首消息），无限重试直到连回；每 `logIntervalSec` 汇总一条重连测试日志。详见 [链路连接逻辑](../weflow-链路连接逻辑（仅上游）.md) §4。
...
### 4.8 `weflow.reconnect.*` —— 自动重连循环（固定间隔，不退避）
...
| 字段 | 默认 | 范围 | 含义 |
|------|------|------|------|
| `intervalSec` | `1` | `1–60` | 每轮重连判定之间的固定间隔（不退避） |
| `logIntervalSec` | `30` | `10–300` | 重连测试日志的汇总周期：每段记录该时间内的测试次数与过程 |

- **校验**：均为正整数；`intervalSec ≥ 1`、`logIntervalSec ≥ 10`。
- **UI 建议**：两个数字框；`intervalSec` 旁注「固定不退避，无限重试」，`logIntervalSec` 旁注「每段汇总一条重连日志」。
...
export interface WeflowConfig {
  ...
  /** 断线后自动重连循环（固定间隔，不退避） */
  reconnect: {
    intervalSec: number     // 默认 1，固定不退避
    logIntervalSec: number  // 默认 30，重连测试日志汇总周期
  }
}
```
...
| `reconnect.intervalSec` | 整数，`1–60` |
| `reconnect.logIntervalSec` | 整数，`10–300` |
...
| `reconnect.*` | 下次断线重连即采用新间隔/日志周期（无需立即断连） |
