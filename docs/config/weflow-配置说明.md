...
### 4.7 `weflow.reconnectIntervalSec` / `weflow.reconnectLogIntervalSec` —— 自动重连循环（固定间隔，不退避）
...
- **校验**：均为正整数；`reconnectIntervalSec ≥ 1`、`reconnectLogIntervalSec ≥ 10`；且 `reconnectLogIntervalSec` **必须大于** `reconnectIntervalSec`（日志周期需能容纳多轮重连，否则一段内攒不满一次重连，汇总无意义）。
...
| `reconnectLogIntervalSec` | 整数，`10–300`；且须**大于** `reconnectIntervalSec` |
