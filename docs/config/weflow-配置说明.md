...
| `weflow.reconnect.initialDelaySec` | 重连起始退避(秒) | integer | `1` | 是 | | FR-CONN-04 |
| `weflow.reconnect.maxDelaySec` | 重连退避上限(秒) | integer | `30` | 是 | | FR-CONN-04 |
| `weflow.reconnect.factor` | 退避倍数 | number | `2` | 是 | | FR-CONN-04 |
| `weflow.reconnect.maxRetries` | 最大重连次数 | integer | `0`（无限） | 是 | | FR-CONN-04 |
| `weflow.reconnect.jitter` | 退避抖动 | boolean | `true` | 否 | | FR-CONN-04（实现增强） |

> 简写对照：需求文档 §10 的「重连退避 `1→30`」= `initialDelaySec:1` → `maxDelaySec:30`；「最大次数 `0`」= `reconnect.maxRetries:0`。
...
### 4.6 `weflow.healthIntervalSec` —— health 探活间隔（秒）

- **含义**：`healthMonitor` 周期调用 health 端点的间隔，用于带外判定 WeFlow 进程是否存活/就绪，并区分两类故障：**WeFlow 未就绪**（health 不通）vs **已连无推送**（health 通但 SSE 久无数据）。异常触发告警通道（FR-ALERT-03）。
- **类型 / 默认**：integer / `30`
- **取值范围**：`5–600`。
- **校验**：正整数。
- **UI 建议**：数字框。

### 4.7 `weflow.reconnect.*` —— 断线重连退避

SSE 断开后自动重连，采用**指数退避 + 上限封顶**（FR-CONN-04）。各字段：

| 字段 | 默认 | 范围 | 含义 |
|------|------|------|------|
| `initialDelaySec` | `1` | `1–60` | 第 1 次重连前的等待秒数（退避起点） |
| `maxDelaySec` | `30` | ≥ `initialDelaySec`，`≤3600` | 退避上限，到顶后维持该间隔 |
| `factor` | `2` | `1.0–10.0` | 每失败一次等待时间的乘数 |
| `maxRetries` | `0` | `≥0` | 最大连续重连次数，**`0` = 无限**；非 0 时达上限停止重连并告警（"重连失败超阈值"），需手动重连 |
| `jitter` | `true` | bool | 是否在退避值上叠加随机抖动（如 ±20%），避免固定节奏惊群。实现增强项，非首版强需求 |

**退避算法**（第 `n` 次重连，`n` 从 1 计）：

```
delay(n) = min(maxDelaySec, initialDelaySec × factor^(n-1))
若 jitter: delay(n) = delay(n) × (1 ± random·0.2)
连接成功后计数器 n 归零；下次断开重新从 initialDelaySec 起步
```

默认 `1→30, factor 2` 的实际节奏：`1s → 2s → 4s → 8s → 16s → 30s → 30s …`（第 6 次起封顶 30s）。

- **校验**：`maxDelaySec ≥ initialDelaySec`；`factor ≥ 1`；`maxRetries ≥ 0`。
- **UI 建议**：「重连退避」子卡片，4 个数字框 + 1 个开关；`maxRetries` 旁注「0 = 无限重连」。
...
export interface WeflowConfig {
  ...
  /** 断线重连退避 */
  reconnect: {
    initialDelaySec: number  // 默认 1
    maxDelaySec: number      // 默认 30
    factor: number           // 默认 2
    maxRetries: number       // 默认 0 = 无限
...
保存时（`PUT /api/config`）逐项校验，不合法返回明确字段级错误提示：
...
| `reconnect.initialDelaySec` | 整数，`1–60` |
| `reconnect.maxDelaySec` | 整数，`≥ initialDelaySec` 且 `≤3600` |
| `reconnect.factor` | 数值，`1.0–10.0` |
| `reconnect.maxRetries` | 整数，`≥0` |
...
| 字段 | 变更行为 |
|------|----------|
| `host` / `port` / `accessToken` / `connectTimeoutSec` / `readTimeoutSec` | **断开并按新参数重连 SSE**（热重连） |
| `healthIntervalSec` | 重置 `healthMonitor` 周期任务 |
| `reconnect.*` | 下次断线重连即采用新退避参数（无需立即断连） |
...
3. **连接状态机**（经 SSE 推前端，FR-CONN-06）：
   `disconnected → connecting → connected`；连上但 health 不通/久无数据 → `weflowNotReady`；断开 → `reconnecting`（携带当前退避秒数、第几次重试）。
