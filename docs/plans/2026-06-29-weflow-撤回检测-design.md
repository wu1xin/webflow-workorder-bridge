...
> **【2026-07-15 修订：本节「零孤儿」取舍已反转】**
> 因「群消息清空重拉」需求（见 [2026-07-15 设计](2026-07-15-群消息清空重拉同步-design.md) §7），`ingestOne` **不再 skip 撤回行**，改为照常走 `serverId` 去重/入队：
> - 原消息活着时已入库 → `serverId` 命中 dedup → duplicate、不重复入队（撤回通知仍由本节的 reconcile 产出，行为不变）；
> - 原消息从未入库（历史撤回 / 清空重拉后原文 REST 已不返回）→ `serverId` 首见 → **作为普通系统消息（`message.new`，含撤回文案）入队转发**，让下游至少看到「这里有条消息被撤回」。
>
> 即：**故意允许历史孤儿撤回入库**（用户拍板，适用所有拉取路径）。系统消息 `localType 10000` → `computeRevocableUntil` 返回 null，入队后不建看守，不与本节 reconcile 冲突；两条路径按「serverId 是否已在 dedup」天然互斥，不会对同一撤回双产出。§4「零历史孤儿撤回」的实现收敛说明在此需求下不再适用。
