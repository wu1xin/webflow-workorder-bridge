...
9）`runCompensation` 接住 newlyAllowed、**把边沿群排除出主循环 candidates**、并在主循环后回灌。把 `await this.syncGroups(sessions)` 改为接收返回值：
```typescript
            const newlyAllowed = await this.syncGroups(sessions)
```
并把 candidates 构造改为排除 newlyAllowed（**关键**：`allowedGroupSessions` 在 `syncGroups` 后读、边沿群此时已放行会被纳入，主循环从全局水位拉它会把其 queue 水位顶到 now，使随后 backfill 读到被顶高的水位、漏掉水位以下空档）：
```typescript
...
```
并在 `this.finish()` 之后 `await` 回灌：
```typescript
            this.advanceWatermark(watermark)
            this.finish()
            await this.backfillNewlyAllowed(newlyAllowed) // 边沿群改由 backfill 从各自旧水位/0 补，主循环不碰它们
