...
>
> **实施状态**：**端到端媒体链路已启用（2026-07-16 起）**。本接口下游侧已实现并可用（上传至阿里云 OSS、幂等复用、安全类型拦截）；`receiveMessage` 信封的 `file` 字段同步启用，下游收到带 `file` 引用的消息会完成媒体落库关联（消息挂媒体 url、按 MIME 细分消息类型、回填媒体↔消息归属）并在工作台前端展示。上游按「先 `uploadMedia` 再在 `receiveMessage` 引用」两步式转发媒体即可（见 §8 与 receiveMessage 文档 v1.3 §4/§5）。
...
> - 单媒体用对象 `{...}`；一条消息多媒体用数组 `[{...},{...}]`（下游消息侧仅展示第一个媒体，其余仅留存关联；上游若有真实多媒体场景请先对齐，见 §12）。
> - 下游只校验 `file_id` 存在性；`url` 等其它字段可带可不带（以 `file_id` 为准）。
> - ✅ **`receiveMessage` 的 `file` 字段已启用（2026-07-16 起）**，上游可按上述方式开始转发媒体消息。
...
| `file` 引用形态 | 一条消息含多媒体时用数组；单媒体用对象。下游两种形态都接受，但**消息侧仅挂第一个媒体**（其余仅留存关联并告警）；上游若存在真实多媒体场景请先对齐 |
| `mediaType` 取值表 | 下游已按服务端探测的 MIME 细分消息类型（image/audio/video→图片/语音/视频，其余→文件），`mediaType`（image/voice/video/file）仅作 MIME 缺失时的回退；如取值超出此表请对齐 |
| OSS 直链可访问性 | 确认 bucket 公共读策略与直链在客户端/后台的可访问性（含防盗链/有效期，如有） |
| ~~媒体链路启用时机~~ | ✅ 已启用（2026-07-16）：`receiveMessage` 的 `file` 字段与下游媒体落库关联均已上线，上游可开始媒体转发 |
...
| v1.1 | 2026-07-16 | **端到端媒体链路启用**：`receiveMessage` 的 `file` 字段生效（该文档 v1.3），下游完成媒体落库关联（消息挂 OSS url、按 MIME 细分图片/语音/视频/文件、回填媒体↔消息归属）。本接口自身契约无变化；§12 中"引用形态/启用时机"确认项更新为已定 |
